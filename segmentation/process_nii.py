...
def process_rescale_intensity(input_folder, output_folder,scale):
    ...
    for file_path in file_paths:
        ...
        v_min, v_max = np.percentile(img_data, (0.2, 99.5))
        #v_min, v_max = np.percentile(img_data, (0.2, 100))
...
def tif2niigz(input_folder, output_folder, specific_string, root_name, target_shape):
    ...
    lower_index = int(np.ceil(z_dim * 0.02))
    upper_index = int(np.floor(z_dim * 0.99))
